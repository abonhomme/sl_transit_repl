...
class SLTransitREPL:
    ...
    LINE_COLORS = defaultdict(lambda: {"color": "white", "name": "Unknown Line"}, {
        ...
        "19": {"color": "green", "name": "Green Line - Hässelby strand to Hagsätra"},
        "40": {"color": TRANSPORT_COLORS["TRAIN"], "name": "Pendeltåg - Uppsala C to Södertälje centrum"},
        "41": {"color": TRANSPORT_COLORS["TRAIN"], "name": "Pendeltåg - Märsta to Södertälje centrum"},
        "42X": {"color": TRANSPORT_COLORS["TRAIN"], "name": "Pendeltåg - Märsta to Nynäshamn"},
        "43": {"color": TRANSPORT_COLORS["TRAIN"], "name": "Pendeltåg - Bålsta to Nynäshamn"},
        "44": {"color": TRANSPORT_COLORS["TRAIN"], "name": "Pendeltåg - Kallhäll to Tumba"},
        "48": {"color": TRANSPORT_COLORS["TRAIN"], "name": "Pendeltåg - Södertälje centrum to Gnesta"}
    })
    ...
    def __init__(self, cache_dir: Optional[str] = None, history_file: Optional[str] = None):
        ...
        if history_file:
            ...
            self.history_file = os.path.join(str(Path.home()), '.sl_transit_repl_history')
    ...
    def _load_sites(self) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            self.console.print(f"[yellow]Warning: Could not read sites cache: {e}[/yellow]")
    ...
    def _save_sites(self, sites: Dict[str, Dict[str, Any]]) -> None:
        ...
        try:
            ...
            self.console.print(f"[yellow]Warning: Could not save sites cache: {e}[/yellow]")

    def _get_site_info(self, site_id: str) -> Optional[Dict[str, Any]]:
        """Get site information and validate it exists."""
        if site_id not in self.sites:
            self.console.print(f"[red]Error: Site ID {site_id} not found in known sites.[/red]")
            self.console.print(f"[yellow]Hint: Delete {self.sites_json} to refresh the sites cache.[/yellow]")
            return None
    ...
    def _build_search_indices(self) -> None:
        ...
        self.idx_by_name: List[Tuple[str, dict]] = []  # List of (normalized_name, site_data) tuples
    ...
    def _find_site_by_id(self, site_id: int) -> Optional[dict]:
    ...
    def _create_site_table(self, sites: Union[List[dict], dict], title: str) -> Table:
    ...
    def _create_departure_table(self, departures: list, show_direction_numbers: bool = False) -> Table:
        ...
        for dep in departures:
            ...
            expected_time = datetime.fromisoformat(dep['expected']) if 'expected' in dep else scheduled_time
    ...
    def _get_departures(self, site_id: int,
                        params: Dict[str, Any],
                       show_direction_numbers: bool = False,
                       debug: bool = False,
                       site_info: Optional[Dict[str, Any]] = None) -> None:
        ...
        try:
            ...
            #cache busting
            api_params['_t'] = str(int(time.time() * 1000))  # Current timestamp in milliseconds
            ...
            if not data.get('departures'):
                self.console.print("[yellow]No departures found for the given criteria.[/yellow]")
                return
            ...
            table = self._create_departure_table(data['departures'], show_direction_numbers)
    ...
    def _parse_query(self, query: str) -> Tuple[bool, Dict[str, str]]:
        ...
        if first_part.startswith('lookup:'):
            ...
            if lookup_type not in ['id', 'name']:
                return False, {'error': f"Invalid lookup type: {lookup_type}. Must be 'id' or 'name'"}
        ...
        for part in parts:
            if ':' not in part:
                return False, {'error': f"Invalid parameter format: {part}. Must be param:value"}
...
[bold yellow]Examples:[/bold yellow]
  ...
  lookup:id 9001                        - Find specific station by ID"""
        ...
        panel = Panel(help_text, title="[bold white]SL Transit REPL Help[/bold white]", border_style="blue")
    ...
    def run(self) -> None:
        ...
        self.console.print("  1002                                              (departure query: just site ID)")
        self.console.print("  1002 line:17 direction:1                          (departure query: with line and direction)")
        self.console.print("  lookup:id 1002                                    (site lookup: find site by ID)")
        self.console.print("  lookup:name odenplan                               (site lookup: find sites by name)")
        ...
        if not self.sites:
            self.console.print("[red]Warning: No sites data available. Some features will be limited.[/red]")
    ...
    def _run_loop(self) -> None:
        ...
        while True:
                ...
                if lookup_type == 'id':
                    try:
                        ...
                        if site:
                            ...
                            self.console.print(f"[red]No site found with ID {site_id}[/red]")
                    except ValueError:
                        self.console.print("[red]Invalid ID format. Please enter a number.[/red]")
                
                elif lookup_type == 'name':
                    sites = self._find_sites_by_substring(search_term)
                    if sites:
                        table = self._create_site_table(sites, f"Sites matching '{search_term}'")
                        self.console.print(table)
                    else:
                        self.console.print(f"[red]No sites found containing '{search_term}'[/red]")
            ...
            api_params = {
                'forecast': result.get('forecast', self.DEFAULT_FORECAST)
            }
